...
def max_subseq(n, l):
    ...
    if l == 0:
        return 0
    # base case 2: l too large
    ## find current number of digits
    num_digits = 1
    while n // pow(10, num_digits) != 0:
        num_digits += 1
    ## if number of digits less than l, return n directly
    if num_digits <= l:
        return n  

    # tree recursive
    else: 
        with_last = 10 * max_subseq((n - n%10)//10, l-1) + n % 10
        without_last = max_subseq((n - n%10)//10, l)
