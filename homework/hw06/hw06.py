class VendingMachine:
    ...
    def vend(self):
        if self.product_stock <= 0:
            ...
            change_message = f' and ${self.balance - self.price} change.' if self.balance > self.price else '.'
            self.balance = 0
            self.product_stock -= 1
            return (f'Here is your {self.product_name}' + change_message)
        else:
...
class Coin:
    ...
    def worth(self):
        ...
        if age > 50:
            extra_percent = age - 50
        else:
            extra_percent = 0
        return self.cents + extra_percent
...
def two_list(vals, amounts):
    ...
    """
    "*** YOUR CODE HERE ***"
    if len(amounts) == 0 and len(vals) == 0:
        return Link.empty
    elif amounts[0] == 0:
        ...
        value = vals[0]
        amounts[0] -= 1
        return Link(value, two_list(vals, amounts))
