...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place:
            ...
            extra_damage = self.damage if (self.health - amount) <= 0 else 0
            for bee in bees:
                bee.reduce_health(amount+extra_damage)
        Ant.reduce_health(self, amount) # 如果先调用 Ant.reduce_health 那么 Fire 直接就被收回了
        # END Problem 5
...
class TankAnt(ContainerAnt):
    """The TankAnt is a container and also deals 1 damage to all bees in its place each turn."""
    ...
    def action(self, gamestate):
        if self.place is not None:
            bees = self.place.bees[:]
            for bee in bees: # 如果迭代的同时修改了元素会导致无法完整遍历
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        super().add_insect(insect) # 也可以说 Place.add_insect(self, insect)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not hasattr(gamestate, "has_queen"):
            gamestate.has_queen = True
            return super(QueenAnt, QueenAnt).construct(gamestate)
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place.exit is not None:
            if place.ant is not None:
                the_ant = place.ant
                while the_ant is not None:
                    if hasattr(the_ant, "buffed") == False:
                        the_ant.damage *= 2
                        the_ant.buffed = True
                    if the_ant.is_container == True and the_ant.ant_contained is not None:
                        the_ant = the_ant.ant_contained
                    else: break
            place = place.exit
        # END Problem 12
